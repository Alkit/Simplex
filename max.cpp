...
bool Max::plane_is_valid()
{
   // cout << "Plan is valid вызов" << endl;
    int i;
    bool result = true;
        for (i = 0; i < (num_v + num_l); i++)
            if (istr[i] < 0) {
...
void Max::print_result_to_file(int it_num)
{
 //   cout << "print result to file вызов" << endl;
    ...
    table << "Решим данную задачу на максимум методом симплексных таблиц.\nПостроим первый опорный план:\n" << endl;
    }
    for (i = 0; i < num_l; i++) {
...
double Max::findalm()
{
    ...
    {
       if (istr[i] < 0)
       for (int j = i; j <  num_v + num_l; j++)
            if (istr[j] < 0){
            if (fabs(istr[i]) < fabs(istr[j]))
                i_lcol = j;
            else
            i_lcol = i;
            }
